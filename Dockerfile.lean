...
RUN apt-get update && \
    ...
    ca-certificates curl iputils-ping python3 python-is-python3 ripgrep gcc python3-dev python3-venv libffi-dev libolm-dev procps git openssh-client docker-cli xz-utils && \
...
RUN touch ./README.md
RUN uv sync --frozen --no-install-project --extra all --extra messaging --extra hindsight
...
# ---------- Project-specific runtime deps (baked so recreate/rebuild never loses behaviour) ----------
# Telegram user-client (telethon + cryptography + python-socks), web search (ddgs),
# ascii-art (pyfiglet). Раньше ставились в рантайме → терялись на recreate. Бейк фиксит.
RUN uv pip install --no-cache-dir telethon cryptography python-socks ddgs pyfiglet && \
    chmod -R a+rX /opt/hermes/.venv
# blogwatcher-cli (Go binary) для скилла blogwatcher; non-fatal, чтобы не валить критичный бейк
RUN curl -fsSL https://github.com/JulienTant/blogwatcher-cli/releases/latest/download/blogwatcher-cli_linux_amd64.tar.gz \
        | tar xz -C /usr/local/bin blogwatcher-cli && chmod 0755 /usr/local/bin/blogwatcher-cli \
    || echo "WARN: blogwatcher-cli install skipped"

...
PLATFORM_HINTS = {
    ...
    "telegram": (
        ...
        "You can send files of any type natively: to deliver a file to the "
        "user, include MEDIA:/absolute/path/to/file in your response. Images "
        "(.png, .jpg, .webp) appear as photos, audio (.ogg) sends as voice "
        "bubbles, videos (.mp4) play inline, and everything else (documents, "
        "archives, packages like .deb, etc.) is attached as a file. Files over "
        "~50MB can't be attached — pack them into an archive (split into "
        "volumes if needed) or share a download link instead. If an attachment "
        "fails, you'll be told why so you can relay it to the user. You can "
        "also include image URLs in markdown format ![alt](url) and they will "
        "be sent as native photos."
    ),
    "discord": (
